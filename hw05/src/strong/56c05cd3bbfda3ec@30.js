...
function _simple2(d3,simple11,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 2000
    ;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(simple11);
...
function _simple11(FileAttachment){return(
FileAttachment("output_11@1.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["output_11@1.json", {url: new URL("../output.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("url_list")).define("url_list", _url_list);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer("simple2")).define("simple2", ["d3","simple11","drag","url_list","invalidation"], _simple2);
  main.variable(observer("simple11")).define("simple11", ["FileAttachment"], _simple11);
  return main;
}
